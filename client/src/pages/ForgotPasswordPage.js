...
function ResetPasswordPage() {
  ...
  // ฟังก์ชันสำหรับขอลิงก์รีเซ็ตรหัสผ่าน (เมื่อไม่มี token)
  const handlePasswordResetRequest = async (e) => {
    ...
    try {
      ...
      setSuccess('ลิงก์สำหรับรีเซ็ตรหัสผ่านถูกส่งไปยังอีเมลของคุณแล้ว');
    } catch (err) {
      console.error('Password reset request error:', err.response?.data?.message || err.message);
      setError(err.response?.data?.message || 'เกิดข้อผิดพลาดในการส่งคำขอ โปรดลองใหม่อีกครั้ง');
    } finally {
      setLoading(false);
    }
  };

  // ฟังก์ชันสำหรับตั้งรหัสผ่านใหม่ (เมื่อมี token)
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'เกิดข้อผิดพลาดในการรีเซ็ตรหัสผ่าน ลิงก์อาจหมดอายุแล้ว');
    } finally {
      setLoading(false);
    }
  };

  const isResettingPassword = !!token; // ตรวจสอบว่ามี token หรือไม่
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-[#F9E9EC] p-4">
      <div className="w-full max-w-sm p-8 space-y-6 bg-transparent">
        ...
        {/* แสดงผลฟอร์มตามเงื่อนไข */}
        {isResettingPassword ? (
          // ฟอร์มสำหรับตั้งรหัสผ่านใหม่
          <form onSubmit={handleResetPassword} className="space-y-4">
            <h2 className="text-xl font-bold text-gray-700 text-center">ตั้งรหัสผ่านใหม่</h2>
            <p className="text-sm text-gray-600 text-center">
              โปรดกรอกรหัสผ่านใหม่ที่คุณต้องการ
            </p>
            {/* Password input */}
          ...
          // ฟอร์มสำหรับขอลิงก์รีเซ็ตรหัสผ่าน
          <form onSubmit={handlePasswordResetRequest} className="space-y-4">
            <h2 className="text-xl font-bold text-gray-700 text-center">รีเซ็ตรหัสผ่าน</h2>
            <p className="text-sm text-gray-600 text-center">
              โปรดกรอกอีเมลที่คุณใช้ลงทะเบียนเพื่อรับลิงก์สำหรับรีเซ็ตรหัสผ่าน
            </p>
            {/* Email input */}
            <div className="relative">
